...
operador informa um motivo (chamado de suporte, verificação de denúncia de
abuso, disputa de cobrança, outro) e a referência do chamado ou da denúncia.
...
**O que fica oculto.** Corpos de e-mail (o detalhe diz "O conteúdo do
e-mail fica oculto no modo de suporte"), corpos de requisição e resposta dos
logs de API, exportações CSV (a rota de exportação responde 403) e todo
segredo: chaves de API, segredos de assinatura de webhook e credenciais SMTP
nunca são devolvidos. Toda alteração é recusada: o servidor responde
