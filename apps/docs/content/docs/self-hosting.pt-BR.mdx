...
description: Implante o MepMail na sua própria infraestrutura com Docker Compose e sua própria conta AWS SES.
---
...
O [MepMail Cloud](https://mepmail.agenciamep.com) é a mesma plataforma,
hospedada.)
...
```sh
mkdir millionsend && cd millionsend
curl -O https://raw.githubusercontent.com/JE4NVRG/millionsend/main/deploy/docker-compose.yml
curl -o .env https://raw.githubusercontent.com/JE4NVRG/millionsend/main/.env.example
```
...
roda `ghcr.io/je4nvrg/mepmail:edge`, que acompanha a `main` (onde todo build
passou pela suíte de testes antes). Publicar uma tag `v*` acrescenta `:1.2.3` e
`:1.2` ao lado. Para segurar uma versão, defina
`MILLIONSEND_IMAGE` no `.env` com uma tag de versão ou um digest imutável
(`ghcr.io/je4nvrg/mepmail@sha256:…`; `docker image ls --digests`
...
```sh
git clone https://github.com/JE4NVRG/millionsend.git
cd millionsend
...
| Variável | Propósito |
| --- | --- |
| `MILLIONSEND_IMAGE` | Só no deploy standalone: qual imagem rodar. O padrão `ghcr.io/je4nvrg/mepmail:edge` acompanha a `main`, então `docker compose pull` é a atualização. Defina uma tag de versão ou um digest imutável `ghcr.io/je4nvrg/mepmail@sha256:…` para segurar uma versão (`MILLIONSEND_BACKUP_IMAGE` fixa o sidecar de backup do mesmo jeito). |
...
A etapa de AWS do `npx @millionsend/setup` cria tudo de que o MepMail
...
servidor; o servidor do MepMail nunca precisa de credenciais de admin.
...
`ghcr.io/je4nvrg/mepmail-backup` (`MILLIONSEND_BACKUP_IMAGE` a fixa, como
...
Nada em uma instância auto-hospedada contata mepmail.agenciamep.com por conta
...
consegue alcançar mepmail.agenciamep.com, ele imprime a página,
[app.mepmail.agenciamep.com/updates?source=self-host](https://app.mepmail.agenciamep.com/updates?source=self-host), e **Configurações →
