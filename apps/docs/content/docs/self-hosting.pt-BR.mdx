...
Esse time é o da própria instância. Dê a ele o plano `system` e ele nunca
tem limite nem cobrança, o selo diz System, a aba Cobrança mostra um aviso
no lugar dos planos e o Stripe nunca escreve na linha dele (numa instância
auto-hospedada os planos não impõem limites, então isso só o rotula; com
`IS_CLOUD=true` também remove os limites de envio, domínios, contatos e
times). O comando não altera nada enquanto o time ainda tiver cliente ou
...
where id = '<id do time>' and stripe_customer_id is null and stripe_subscription_id is null;
```
...
lida por um modelo de linguagem depois que o SES o recebeu, pontuada de 0 a
100 contra uma rubrica fixa e incorporada a um risco por equipe que o
operador vê em Confiança e segurança. Nada no caminho de envio espera por
...
equipe nos seus primeiros dias, pausa os broadcasts quando o risco passa da
...
**Para ligar**, no ambiente do worker (um reinício aplica):

```bash
# Bedrock Converse, com as credenciais ou o papel da AWS que o SES já usa.
ABUSE_JUDGE=bedrock
ABUSE_JUDGE_MODEL=amazon.nova-lite-v1:0
ABUSE_JUDGE_REGION=us-east-1

# Ou qualquer endpoint de chat completions compatível com a OpenAI.
ABUSE_JUDGE=openai
ABUSE_JUDGE_MODEL=gpt-5-nano
ABUSE_JUDGE_API_KEY=...
ABUSE_JUDGE_BASE_URL=https://api.openai.com/v1

# Ou a Messages API da Anthropic.
ABUSE_JUDGE=anthropic
ABUSE_JUDGE_MODEL=claude-haiku-4-5
ABUSE_JUDGE_API_KEY=...

# Timeout por chamada (padrão 20000 ms).
ABUSE_JUDGE_TIMEOUT_MS=20000
```

Um provedor sem o modelo, ou um provedor hospedado sem a chave, falha a
inicialização. A rubrica está no repositório
(`packages/core/src/abuse-judge/rubric.ts`); ela foi escolhida em um corpus
rotulado com o Nova Lite, e modelos pequenos da classe do Nova Micro não
seguram uma rubrica desse tamanho e geram falsos positivos, então use o Nova
Lite ou maior (ou um modelo da classe Haiku) em vez de encurtar a rubrica.
Confira os termos de retenção do seu provedor hospedado antes de apontar o
juiz para ele: o texto abaixo chega até ele.

**Exatamente o que o modelo vê**, montado em memória a cada chamada e nunca
...
por e-mail quando mais de 20% das amostras de uma hora ficaram sem
julgamento.
