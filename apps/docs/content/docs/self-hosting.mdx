...
Migrations run on boot, so that is the whole upgrade for a small instance. The
compose file runs
...
but a spam report is about one sender's mail — MillionSend suppresses it for
...
`millionsend-ses` policy) before turning the flag on.
...
That team is the instance's own. Give it the `system` plan and it is never
capped or billed, its badge reads System, the Billing tab shows a notice
instead of plans, and Stripe never writes its row (on a self-hosted instance
plans carry no limits, so this only labels it; with `IS_CLOUD=true` it also
lifts the send, domain, contact and team caps). The statement changes
nothing while the team still has a Stripe customer or subscription:
...
address to MillionSend release notes; that is the wizard on your machine
posting your answer, and a confirmation link is sent before anything is
stored. When it cannot reach millionsend.com it prints the page instead,
[app.millionsend.com/updates?source=self-host](https://app.millionsend.com/updates?source=self-host), and **Settings →
...
alert line, and, for a team in its first days only, pauses broadcasts when
the risk passes the pause line and a sampled message scored 90 or more
...
**Turning it on** in the worker's environment (a restart applies it):
...
the boot. The rubric ships in the repository (`packages/core/src/abuse-judge/rubric.ts`);
it was chosen on a labelled corpus with Nova Lite, and small models of the
Nova Micro class cannot hold a rubric this long and false-positive on it, so
use Nova Lite or larger (or a Haiku-class model) rather than shortening the
rubric. Check your hosted provider's retention terms before pointing the
judge at it: the text below reaches it.
