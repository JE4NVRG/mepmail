...
ticket, verifying an abuse report, billing dispute, other) and the ticket or
report reference. The session rides on the operator's own login; no session
is ever minted for the owner.
...
hidden in support view"), API log request and response bodies, CSV exports
(the export route answers 403), and every secret: API keys, webhook signing
secrets and SMTP credentials are never returned. Every change is refused:
