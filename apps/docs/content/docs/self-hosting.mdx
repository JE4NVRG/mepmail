---
title: Self-hosting
description: Deploy MepMail on your own infrastructure with Docker Compose and your own AWS SES account.
---
...
infrastructure? [MepMail Cloud](https://mepmail.agenciamep.com) is the same
platform, hosted.)
...
| Variable | Purpose |
| --- | --- |
| `MILLIONSEND_IMAGE` | Standalone deploy only: which image to run. The default `ghcr.io/je4nvrg/mepmail:edge` follows `main`, so `docker compose pull` is the upgrade. Set a version tag or an immutable `ghcr.io/je4nvrg/mepmail@sha256:…` digest to hold a version (`MILLIONSEND_BACKUP_IMAGE` pins the backup sidecar the same way). |
...
The AWS step of `npx @millionsend/setup` creates everything MepMail needs
...
server; the MepMail server never needs admin credentials. It verifies your
...
Nothing on a self-hosted instance contacts mepmail.agenciamep.com on its own. The
...
stored. When it cannot reach mepmail.agenciamep.com it prints the page instead,
[app.mepmail.agenciamep.com/updates?source=self-host](https://app.mepmail.agenciamep.com/updates?source=self-host), and **Settings →
